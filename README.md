<div align="center">

<!-- Animated wave banner -->
<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:0d1117,50:1a1f35,100:58A6FF&height=220&section=header&text=Akshit%20Sharma&fontSize=65&fontColor=ffffff&fontAlignY=38&desc=Computer%20Vision%20%7C%20Machine%20Learning%20%7C%20AI%20Systems&descAlignY=58&descSize=20&animation=fadeIn" />

<!-- Typing animation -->
<img src="https://readme-typing-svg.demolab.com/?font=Fira+Code&weight=700&size=24&duration=3000&pause=1200&color=58A6FF&center=true&vCenter=true&width=750&lines=Building+deployable+AI+systems+%F0%9F%9A%80;Computer+Vision+%7C+Deep+Learning+%7C+MLOps;B.Tech+CSE+%40+SRMIST;Turning+research+into+real+products+%F0%9F%94%A5" alt="Typing SVG" />

<br/>

<a href="https://www.linkedin.com/in/akshitsharma02/"><img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" /></a>
&nbsp;
<a href="mailto:akshitt27@gmail.com"><img src="https://img.shields.io/badge/Email-akshitt27%40gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white" /></a>
&nbsp;
<a href="https://github.com/akshiit02"><img src="https://img.shields.io/badge/GitHub-akshiit02-181717?style=for-the-badge&logo=github&logoColor=white" /></a>

</div>

<br/>

---

## 👨‍💻 About Me

<br/>

<table width="100%">
<tr>
<td width="48%" valign="top">

```python
class AkshitSharma:
    def __init__(self):
        self.name       = "Akshit Sharma"
        self.university = "SRMIST — B.Tech CSE"
        self.focus      = ["Computer Vision",
                           "Applied ML",
                           "MLOps"]
        self.current    = "Building deployable AI pipelines"
        self.interest   = "Where research meets production"

    def say_hi(self):
        print("Thanks for dropping by!")
        print("Let's build something great.")

me = AkshitSharma()
me.say_hi()
```

<br/>

- 🔭 Turning research-heavy ML into things people can actually use
- 🌱 Currently exploring multi-task learning & real-time CV inference
- 💬 Ask me about **PyTorch · FastAPI · YOLO · Satellite ML**
- 📫 **akshitt27@gmail.com**

</td>
<td width="4%"></td>
<td width="48%" valign="top" align="center">

<img src="https://github-readme-stats.vercel.app/api?username=akshiit02&show_icons=true&theme=tokyonight&title_color=58A6FF&icon_color=58A6FF&text_color=C9D1D9&bg_color=0D1117&hide_border=true&count_private=true&rank_icon=github&card_width=450" width="100%" />

<br/><br/>

<img src="https://streak-stats.demolab.com/?user=akshiit02&theme=tokyonight&hide_border=true&background=0D1117&ring=58A6FF&fire=FF6B6B&currStreakLabel=58A6FF&sideLabels=C9D1D9&dates=8B949E&card_width=450" width="100%" />

<br/><br/>

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=akshiit02&layout=compact&theme=tokyonight&title_color=58A6FF&text_color=C9D1D9&bg_color=0D1117&hide_border=true&langs_count=6&card_width=450" width="100%" />

</td>
</tr>
</table>

---

## 🚀 Featured Projects

<div align="center">

| 🏷️ Project | 📝 What It Does | 🛠️ Stack | 🔗 |
|:---|:---|:---|:---:|
| 🔥 **Forest Fire Prediction System** | Multi-task neural network (STW-BP) predicting wildfire occurrence *and* intensity across India using NASA MODIS satellite data — custom adaptive feature-weighting, ~98% accuracy | `TensorFlow` `XGBoost` `Scikit-learn` | [→ Repo](https://github.com/akshiit02/Forest-Fire-Prediction-System-using-Multi-Task-STW-BP) |
| 🌾 **AI Vision System — Crop, Soil & Pest** | End-to-end crop disease detection, pest identification & soil analysis — FastAPI backend with working web frontend | `PyTorch` `FastAPI` `MobileNetV2` `EfficientNet` | [→ Repo](https://github.com/akshiit02/AI-Vision-System-for-Crop-Soil-and-Pest-Monitoring) |
| 🚶 **Pedestrian Detector** | Real-time pedestrian detection in video using YOLOv4-tiny + OpenCV DNN, with non-max suppression | `Python` `OpenCV` `YOLOv4` | [→ Repo](https://github.com/akshiit02/Pedestrian-Detector) |
| 🌐 **TradeConnect** | Responsive export-sourcing brokerage site with scroll animations & contact form | `HTML` `CSS` `JavaScript` | [→ Repo](https://github.com/akshiit02/TradeConnect) · [▶ Live](https://akshiit02.github.io/TradeConnect/) |
| 🎮 **2048 Game** | Browser-based 2048 clone from scratch — vanilla JS, live score, arrow-key controls | `JavaScript` `HTML` `CSS` | [→ Repo](https://github.com/akshiit02/2048-Game) · [▶ Live](https://akshiit02.github.io/2048-Game/) |

</div>

---

## 🛠️ Tech Stack

<div align="center">

**Languages**

![Languages](https://skillicons.dev/icons?i=python,cpp,java,c,js,html,css&perline=7)

**ML · CV · Data Science**

![ML](https://skillicons.dev/icons?i=pytorch,opencv,tensorflow,sklearn&perline=7)

**Backend · DevOps · Tools**

![Tools](https://skillicons.dev/icons?i=fastapi,spring,mysql,php,docker,git,linux,vscode&perline=8)

</div>

---

## 📊 GitHub Activity

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=akshiit02&theme=tokyo-night&bg_color=0D1117&color=58A6FF&line=58A6FF&point=FF6B6B&area=true&hide_border=true&area_color=1a2a4a" width="100%" />

</div>

---

## 🏆 GitHub Trophies

<div align="center">

<img src="https://github-profile-trophy.vercel.app/?username=akshiit02&theme=tokyonight&no-frame=false&no-bg=false&margin-w=10&column=6" width="96%" />

</div>

---

## 🐍 Contribution Snake

<div align="center">

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/akshiit02/akshiit02/output/github-contribution-grid-snake-dark.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/akshiit02/akshiit02/output/github-contribution-grid-snake.svg" />
  <img alt="Snake animation" src="https://raw.githubusercontent.com/akshiit02/akshiit02/output/github-contribution-grid-snake-dark.svg" />
</picture>

</div>

---

<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:58A6FF,50:1a1f35,100:0d1117&height=120&section=footer" />

</div>
